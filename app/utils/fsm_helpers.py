...
def get_prompt_for_state(state: State, context: Any) -> Tuple[str, Optional[Any]]:
    ...
    if state == Registration.waiting_for_edit_first_name:
        return "✍️ Введите новое имя:", None
    if state == Registration.waiting_for_edit_last_name:
        return "✍️ Введите новую фамилию:", None
    if state == Registration.waiting_for_edit_gender:
        return "Выберите ваш пол:", get_gender_keyboard()
    if state == Registration.waiting_for_edit_birth_date:
        return "📅 Введите новую дату рождения в формате ДД.ММ.ГГГГ (например, 25.12.1990):", None
    if state == Registration.waiting_for_edit_email:
